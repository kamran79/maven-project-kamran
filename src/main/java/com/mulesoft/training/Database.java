...
public class Database implements InitializingBean {
	...
	public void testMethod() {
		System.out.println("kamran test.");
	}
	...
	public void afterPropertiesSet() throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				if (rs.getString(3).equalsIgnoreCase("FLIGHTS"))
					i = 1; // set a marker that this table already exists
			}
			logger.debug("&&&& - DB Init - &&&&");
			Statement stmt = conn.createStatement();
			if (i != 1) {
				stmt.executeUpdate(
						"CREATE TABLE FLIGHTS(ID INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 0)  NOT NULL PRIMARY KEY,"
								+ "PRICE INT," + "DESTINATION VARCHAR(255)," + "ORIGIN VARCHAR(255))");
